...
func TestOtlpToInternalReadOnly(t *testing.T) {
	...
	numberDataPoints := dsd.DataPoints()
	assert.EqualValues(t, 2, numberDataPoints.Len())
	// First point
	assert.EqualValues(t, startTime, numberDataPoints.At(0).StartTimestamp())
	assert.EqualValues(t, endTime, numberDataPoints.At(0).Timestamp())
	assert.EqualValues(t, 123.1, numberDataPoints.At(0).Value())
	assert.EqualValues(t, NewStringMap().InitFromMap(map[string]string{"key0": "value0"}), numberDataPoints.At(0).LabelsMap())
	// Second point
	assert.EqualValues(t, startTime, numberDataPoints.At(1).StartTimestamp())
	assert.EqualValues(t, endTime, numberDataPoints.At(1).Timestamp())
	assert.EqualValues(t, 456.1, numberDataPoints.At(1).Value())
	assert.EqualValues(t, NewStringMap().InitFromMap(map[string]string{"key1": "value1"}), numberDataPoints.At(1).LabelsMap())
...
func TestOtlpToFromInternalDoubleSumMutating(t *testing.T) {
	...
	numberDataPoints := dsd.DataPoints()
	numberDataPoints.At(0).SetStartTimestamp(Timestamp(startTime + 1))
	assert.EqualValues(t, startTime+1, numberDataPoints.At(0).StartTimestamp())
	numberDataPoints.At(0).SetTimestamp(Timestamp(endTime + 1))
	assert.EqualValues(t, endTime+1, numberDataPoints.At(0).Timestamp())
	numberDataPoints.At(0).SetValue(124.1)
	assert.EqualValues(t, 124.1, numberDataPoints.At(0).Value())
	numberDataPoints.At(0).LabelsMap().Delete("key0")
	numberDataPoints.At(0).LabelsMap().Upsert("k", "v")
	assert.EqualValues(t, newLabels, numberDataPoints.At(0).LabelsMap())
